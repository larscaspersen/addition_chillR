#' Generates relative climate change scenarios based on extrated CMIP6 data
#' 
#' Takes the extracted CMIP6 data and returns climate change scenarios, which can be
#' then used to generate weather data. 
#' 
#' @param downloaded_list list of dataframes, generated using the extract_cmip6_data function. Elements are named after the shared socioeconomic
#' pathway 'SSP' and global climate model (GCM)
#' 
#' @param years_local_weather by default set NULL. If provided, it states the earliest and latest baseline year for which the relative scenario should be generated.
#' The same values will be used for all weather stations in this case. Either years_local_weather or weather_list needs to be provided.
#' 
#' @param weather_list by default set NULL. If provided, should be a list of data.frames containing the locally observed weather. Is used to determine
#' the earliest and latest year for which we have observations. 
#' 
#' @param times numeric vector, states the future years, for which the climate change scenario should be generated. By default set to c(2050, 2085)
#' 
#' @param baseline_year_relative_change numeric, states for which year within the downloaded CMIP6 data the relative change should be calculated. By default set to 2022
#' 
#' @param baseline_window_width numeric, sets the window width of the running mean calculation for the mean temperatures of the year indicated by baseline_year_relative_change
#' 
#' @param future_window_width numeric, sets the window width of the running mean calculation for the mean temperatures of the year indicated by times
#' 
#' 
#' @return list of relative climate change scenarios. It is ordered in three levels: 1) locations 2) SSP_GCM and 3) timpoint of interest
#' 
#' @author Lars Caspersen
#' 
#' @examples \dontrun{
# scenario<-c("ssp1_2_6", "ssp2_4_5", "ssp3_7_0", "ssp5_8_5")
# purrr::map(scenario, run_download_cmip6, area = c(52, -7, 33, 8))
#' station <- data.frame(
...
#' extracted <- extract_cmip6_data_antonio(stations = station)
#' scenario <- gen_rel_change_scenario(extracted, years_local_weather = c(1992, 2021))
#' }
#' 
#' @importFrom purrr map
#' @importFrom chillR temperature_scenario_from_records
