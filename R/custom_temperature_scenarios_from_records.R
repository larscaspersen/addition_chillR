...
#' c("Month","Day","Year","Tmin","Tmax", "Prec").
...
#' @param prec_output by default NULL, otherwise needs to be a character. If set to "monthly_sum"
#' the scenario contains monthly precipitation sums. If set to "monthly_mean_daily_prec"
#' the scenario contains the mean daily precipitation of a month.
          ...
          scen_type = "running_mean", runn_mean = 15, prec_output = 'monthly_mean_daily_prec') 
{
  ...
  if (!"Tmin" %in% colnames(weather) | !"Tmax" %in% colnames(weather) | 
      ...
      !"Year" %in% colnames(weather) | !"Prec" %in% colnames(weather)) 
    return(warning("Error - required columns missing ('Day','Month','Year','Tmin', 'Tmax' or 'Prec)"))
